        ...
        if self.results.multi_face_landmarks:
            for faceLms in self.results.multi_face_landmarks:
                ...
                for id, lm in enumerate(faceLms.landmark):
                    ...
                    #cv2.putText(img, str(id), (x, y), cv2.FONT_HERSHEY_PLAIN, 0.7, (0, 255, 0), 1) #SHOWS ALL VALUES/IDS INSTEAD OF DOTS
                    #print(x,y)
                    face.append([x,y])
                faces.append(face)
        return img, faces
...
def main():
    ...
    while True:
        ...
        img,faces = detector.findFaceMesh(img) #add ,False paramater for now drawing
        # if len(faces)!=0:
        #     print(len(faces))
